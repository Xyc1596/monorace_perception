...
from .corner import CornerFromMask, Corner
from .line_segment import LineSegment


class CornerDetector:
    def __init__(self, intrinsic_mat: NDArray, extrinsic_mat: NDArray):
        self.__INTRINSIC_MAT: NDArray = intrinsic_mat
        self.__ROT_B2C = Rotation.from_matrix(extrinsic_mat[:3, :3])
        self.__EXT_FACTOR: float = 5 / 3
        """线段长度缩放因子"""
        self.__MATCH_DIST_THRESH: int = 100
        """如果先验角点与候选角点像素距离大于此值，则拒绝匹配"""
        self.__RANSAC_THRESH: float = 5.
        """RANSAC 内点阈值（像素）"""
        self.__MAX_TRANSLATION: int = 150
        """RANSAC 任何方向上的平移超过此值则拒绝解"""

        self.__lsd: cv2.LineSegmentDetector = cv2.createLineSegmentDetector(
            scale=0.8,
            sigma_scale=0.8,
            quant=25.0,
            ang_th=30.0
    ...
    def _derotate_mask(self, mask: NDArray[np.uint8], body_quat_est: NDArray) -> NDArray[np.uint8]:
        """
        掩码去旋转（使图像中的垂直轴与世界向上方向对齐） TODO: 去旋转是否考虑偏航和俯仰？旋转中心？
        :param mask: 二值化掩码图像 [H, W]（0/255）
        :param body_quat_est: 状态估计的无人机姿态四元数（I2B）
        :return: 去旋转的掩码图像 [H, W]（0/255）
        """
        rot_i2c = self.__ROT_B2C * Rotation.from_quat(body_quat_est)
        # 惯性系向上向量投影到图像平面
        vec_up = self.__INTRINSIC_MAT @ rot_i2c.apply([0, 0, 1])
        ...
        return cv2.warpAffine(
            mask,
            cv2.getRotationMatrix2D(center, rot_deg, 1.),
            (w, h),
    ...
    def _detect_lines(self, mask: NDArray[np.uint8]) -> List[LineSegment]:
        ...
        :param mask: 去旋转后的二值图像（0/255）
        :return: 线段对象列表
        """
        # lines 格式: N x 1 x 4 (x1,y1,x2,y2)
        lines = self.__lsd.detect(mask)[0]
        if lines is None:
            return []
        segments = []
        for line in lines:  # (x1, y1, x2, y2)
            segments.append(LineSegment.of_line(line[0], self.__EXT_FACTOR))
        return segments
    ...
    def _compute_corner_candidates(
        ...
        :param extended_segments: 扩展线段列表
        :param mask: 去旋转后的二值图像（0/255）
    ...
    def _match(
        ...
        :param prior_corners: 先验角点
        :param candidate_corners: 候选角点
        ...
        for prior_corner in prior_corners:
            ...
            for candidate_corner in candidate_corners:
                ...
                if dist < self.__MATCH_DIST_THRESH and dist < best_dist:
                    best_dist = dist
    ...
    def _ransac_transform(
        ...
        :param matches: 根据描述符完全匹配 + 距离约束建立的角点匹配对 (prior, candidate)
        :return: 经过变换后的四个角点坐标（按照先验顺序），若失败则返回None
        """
        ...
        transform, inliers = cv2.estimateAffinePartial2D(
            ...
            method=cv2.RANSAC,
            ransacReprojThreshold=self.__RANSAC_THRESH
        )
        ...
        if abs(tx) > self.__MAX_TRANSLATION or abs(ty) > self.__MAX_TRANSLATION:
            return None
    ...
    def detect(self, mask: NDArray[np.uint8], body_quat_est: NDArray, prior_corners: List[Corner]):
        """
        QuAdGate
        :param mask: 单个门框的二值化掩码图像 [H, W]（0/255）
        :param body_quat_est: 状态估计的无人机姿态四元数（I2B）
        :param prior_corners: 上一帧或模板中的四个门框角点
        :return: 当前帧中四个门框角点坐标 [4,2]，若失败则返回None
        """
        # 1. 掩码去旋转
        derotated_mask = self._derotate_mask(mask, body_quat_est)
        # 2. LSD 线段检测
        ...
        transformed_prior_corners = self._ransac_transform(prior_corners, matches)
